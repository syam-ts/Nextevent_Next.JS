...
const signupFunction = async (formData: {
  ...
  const res = await axios.post(
    "http://localhost:3001/api/organizer/signup",
    formData
  );
  return res.data;
};
...
const page = () => {
  ...
  const { mutate } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if(data.success) route.push('/organizer/login')
    },
    onError: (error) => {
  ...
  return (
    <div> 
        <section className="min-h-screen flex items-stretch text-white ">
          ...
          <div className="lg:w-1/2 w-full flex bg-[#335c67] items-center justify-center text-center md:px-16 px-0 z-0">
